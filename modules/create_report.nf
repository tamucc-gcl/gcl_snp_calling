...
def num(x, digits=2):
    ...
    try:
        xf = float(x)
        if math.isfinite(xf):
            if abs(xf) >= 1000 and digits == 0:
                return f"{int(round(xf)):,}"
            if abs(xf) >= 1000 and digits > 0 and xf.is_integer():
                return f"{int(xf):,}"
...
# ------------------------------------------------------------
try:
    sample_rows = read_tsv(sample_qc_derived)
except Exception:
    sample_rows = []

try:
    locus_rows = read_tsv(locus_qc_derived)
except Exception:
    locus_rows = []

try:
    worst_sample_rows = read_tsv(worst_samples)
except Exception:
    worst_sample_rows = []

try:
    worst_locus_rows = read_tsv(worst_loci)
except Exception:
    worst_locus_rows = []
...
if n_records is not None:
    interpretation.append(f"The raw callset contains **{n_records} variant records** across **{summary_numbers.get('number of samples', metrics.get('samples_total', 'NA'))} samples**.")

if ts_tv and ts_tv.get('ratio') not in (None, '', 'NA'):
    interpretation.append(f"The **transition/transversion ratio is {ts_tv['ratio']}**, which is a useful high-level check on callset plausibility.")
